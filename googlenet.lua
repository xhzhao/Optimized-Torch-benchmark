...
local function inception(depth_dim, input_size, config, lib)
   ...
   local concat = nn.Concat(depth_dim)

   if config[1][1] ~= 0 then
      local conv1 = nn.Sequential()
      conv1:add(SpatialConvolution(input_size, config[1][1],1,1,1,1)):add(ReLU(true))
      concat:add(conv1)
   end

   local conv3 = nn.Sequential()
   conv3:add(SpatialConvolution(  input_size, config[2][1],1,1,1,1)):add(ReLU(true))
   conv3:add(SpatialConvolution(config[2][1], config[2][2],3,3,1,1,1,1)):add(ReLU(true))
   concat:add(conv3)
   local conv3xx = nn.Sequential()
   conv3xx:add(SpatialConvolution(  input_size, config[3][1],1,1,1,1)):add(ReLU(true))
   conv3xx:add(SpatialConvolution(config[3][1], config[3][2],3,3,1,1,1,1)):add(ReLU(true))
   conv3xx:add(SpatialConvolution(config[3][2], config[3][2],3,3,1,1,1,1)):add(ReLU(true))
   concat:add(conv3xx)

   local pool = nn.Sequential()
   pool:add(nn.SpatialZeroPadding(1,1,1,1)) -- remove after getting nn R2 into fbcode
   pool:add(SpatialMaxPooling(3,3,1,1):ceil())
   if config[4][2] ~= 0 then
      pool:add(SpatialConvolution(input_size, config[4][2],1,1,1,1)):add(ReLU(true))
   end
   concat:add(pool)
