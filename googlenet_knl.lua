...
-- if this is 512->24->64, do not use MKLDNN op, this follow caffe
   if input_size==512 and config[3][1]==24 then 
        local conv5 = nn.Sequential()
        conv5:add(nn.SpatialConvolutionMM(input_size, config[3][1], 1, 1)):add(ReLU(true))
        conv5:add(nn.SpatialConvolutionMM(config[3][1], config[3][2], 5, 5, 1, 1, 2, 2)):add(ReLU(true))
